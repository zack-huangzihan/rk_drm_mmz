...
static MB_BLK create_blk_from_fd_and_handle(int buf_fd, int handle, RK_U32 len, uint32_t flags)
{
    // get buffer length
    if (len == 0) {
        off_t _size = dmabuf_get_size(buf_fd);
        if (_size == (off_t) -1) {
            printf("get buffer length failed: %s \n", strerror(errno));
            return (MB_BLK)NULL;
        }
        len = _size;
    }
    ...
    // get physic addr
    uint32_t paddr = (uint32_t)-1;
    if (drm_get_phys(handle, &paddr) < 0) {
        printf("get physical address fail \n");
    }

    // get virt addr
    void* vaddr = dmabuf_mmap(buf_fd, 0, len);
    if (vaddr == NULL) {
...
RK_S32 RK_MPI_MMZ_Alloc(MB_BLK *pBlk, RK_U32 u32Len, RK_U32 u32Flags) {
    ...
    int handle = 0;
    int buf_fd = 0;
    if (drm_alloc(u32Len, is_cma, is_cacheable, reinterpret_cast<uint32_t *>(&handle)) < 0) {
        return -1;
    }
    if (drm_handle_to_fd(handle, &buf_fd) < 0) {
        return -1;
    }

    MB_BLK mb = create_blk_from_fd_and_handle(buf_fd, handle, u32Len, u32Flags);
